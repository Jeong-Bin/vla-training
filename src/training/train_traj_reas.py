...
class TrajTrainSet:
    ...
    def __getitem__(self, i):
        ...
        hist = history_for(rec, self.temporal_on)          # 과거 8뷰(temporal_on일 때만)
        enc, labels, plen = build_model_inputs(self.processor, rec["images"], prompt, reasoning, hist)
...
class TrajReasVLA(nn.Module):
    ...
    def forward(self, batch):
        ...
        # flow/lm_loss는 로깅 전용 → detach해 반환(DDP가 loss 외 별도 grad-root로 오인하지 않게).
...
def evaluate(vlm, dit_mod, processor, val_records, prompt_tpl, normalizer, reas_weight, device, joint,
    ...
    with torch.no_grad():
        for i in my:
            ...
            c, lml, mem, mem_mask = encode_and_lm_loss(vlm, processor, rec["images"], prompt, reasoning,
    ...
    g_flow, g_tot, g_lm, g_n, g_lm_n = ddp.all_reduce_sum([flow_sum, tot_sum, lm_sum, n, lm_n])
...
def main() -> None:
    ...
    #   ⚠️ cross_attn이면 DiT가 pooled cond을 안 쓰므로(scene은 cross-attn으로 유입) 이 적합을 스킵한다
    #      (256회 VLM forward 절약; cond_mean/std 버퍼는 기본 0/1=항등으로 남고 forward에서 미사용).
    if not args.cross_attn:
        ...
        log("cross_attn=ON → pooled cond 미사용, cond normalizer fit 스킵(scene은 cross-attention으로 유입)")
    ...
    # DiT와 VLM은 학습률을 분리(VLM은 보통 더 작게).
    ...
    while not done:
        ...
        for s, batch in enumerate(loader):
            ...
            with (ddp_model.no_sync() if use_no_sync else nullcontext()):
                # 래핑 forward(메인 GPU): VLM forward+condition → DiT flow_loss(+λ·LM +0-더미). load+preprocess는 워커가 끝냄.
                loss, flow, lm_loss = ddp_model(batch)       # (total, flow(detached), 배치 LM_loss or None)
